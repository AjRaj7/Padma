...
// Calculate total savings amount (unallocated income)
export const calculateSavingsAmount = (monthlyIncome: number, streams: Stream[]): number => {
  const totalAllocated = streams.reduce((sum, s) => sum + s.originalAmount, 0);
  return Math.max(0, monthlyIncome - totalAllocated);
};
...
export const calculateStreamBalance = (stream: Stream, transactions: Transaction[]): number => {
  ...
  if (stream.isGoal) {
    // For savings goals, show how much has been saved (positive growth)
    return totalSpent;
  } else {
...
export const calculateStreamProgress = (stream: Stream, transactions: Transaction[]): number => {
  ...
  if (stream.isGoal) {
    // For savings goals, progress is how much saved towards the goal
...
export const calculateSavingsRate = (state: AppState): number => {
  ...
  const savingsStreams = streams.filter(s => s.isGoal);
  const totalSaved = transactions
    .filter(t => savingsStreams.some(s => s.id === t.streamId))
    .reduce((sum, t) => sum + t.amount, 0);
    
  return (totalSaved / user.monthlyIncome) * 100;
};
